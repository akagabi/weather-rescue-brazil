...
this script's default `probe` mode runs exactly the 2 structurally-worst
sheets from the G1 gate (bench/g1/gemini-flash.json, the gemini-3.5-flash
zero-shot run) and compares g2b's cell_acc/structural_err_rate against that
same G1 baseline on those same 2 sheets. It does NOT run all 9 sheets - that
is Task 3, gated on this probe clearing the checkpoint bar, and is a
separate controller decision (see the plan's Task 2 checkpoint note).

Sheets are processed in chronological (page) order so `reconcile_period`
has a real anchor: this station's gold sheets are drawn from a corpus whose
selected months run consecutively in most stretches (see gold/SELECTION.md)
- notably 14_22 (1885-12) immediately precedes 14_41 (1886-01), which is
exactly what lets reconcile_period override 14_41's g2b period read using
14_22's already-reconciled period as `expected_prev`. The very first sheet
processed in a run has no prior anchor (expected_prev=None): its period
read is only checked against the known corpus year range, not overridden.
...
def run_g2b_on_sheet(
    sheet: Sheet, meter: CostMeter, expected_prev: tuple[int, int] | None,
) -> tuple[dict, tuple[int, int]]:
    """Run the full g2b pipeline (period call -> reconcile -> table call ->
    barometer reconstruction -> assemble) on one sheet, score it against
    gold, and return (result_entry, reconciled_period) - the reconciled
    period becomes the next call's `expected_prev` anchor."""
...
def main() -> None:
    ...
    if mode != "probe":
        print(f"unknown mode {mode!r}; only 'probe' is implemented "
              "(Task 2 scope - full run is Task 3, gated on this probe)", file=sys.stderr)
        sys.exit(2)
    ...
    results: list[dict] = []
    expected_prev: tuple[int, int] | None = None
    for sheet in probe_sheets:
        gold_file = f"14_{sheet.page}.json"
        try:
            entry, reconciled = run_g2b_on_sheet(sheet, meter, expected_prev)
        except CapExceeded:
            raise
